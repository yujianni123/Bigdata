...
int main(int argc, char *argv[])
{
    ...
    // 设置渲染器绘画颜色
    SDL_SetRenderDrawColor(My_render, 0, 0, 255, 255);

    // 使用渲染器绘制点
    SDL_RenderDrawPoint(My_render, 512, 300);

    for (int i = 512; i < 800; i += 5)
    {
        SDL_RenderDrawPoint(My_render, i, 300);
    }

    // 使用渲染器绘制线
    // SDL_RenderDrawLine(My_render, 0, 0, 212, 400);

    SDL_Point My_points[5] = {{100, 100}, {50, 555}, {895, 456}, {123, 79}, {456, 239}};
    SDL_RenderDrawLines(My_render, My_points, 5);
